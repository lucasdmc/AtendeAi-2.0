...
class ConversationController {
  ...
  async processWhatsAppMessage(req, res) {
    try {
      ...
      // Montar resposta baseada no orquestrador
      if (processingResult.type === 'response') {
        res.json({
          success: true,
          data: {
            response: processingResult.response.content,
            intent: processingResult.intent,
            confidence: processingResult.confidence,
            metadata: processingResult.metadata
          }
        });
      } else if (processingResult.type === 'service_routing' || processingResult.type === 'routed') {
        res.json({
          success: true,
          data: {
            response: 'Sua solicitação foi encaminhada para o serviço apropriado.',
            intent: processingResult.intent,
            confidence: processingResult.confidence,
            metadata: processingResult.routing
          }
        });
      } else {
        res.json({
          success: true,
          data: {
            response: processingResult.message || 'Desculpe, não consegui processar sua mensagem. Tente novamente.',
            intent: 'unknown',
            confidence: 0,
            metadata: { error: true }
          }
  ...
  async handleAIResponse(processingResult, conversation, message) {
    ...
    return {
      type: 'ai_response',
      content: processingResult.response?.content || 'Olá! Como posso ajudar?',
      confidence: processingResult.confidence
    };
  }
